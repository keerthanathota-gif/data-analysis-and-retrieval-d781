...
const CFRDashboard = () => {
  ...
  const runPipeline = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://localhost:8000/search/pipeline/run',
        { urls },
        getAuthHeaders()
      );
  ...
  const checkPipelineStatus = async () => {
    try {
      const response = await axios.get('http://localhost:8000/search/pipeline/status', getAuthHeaders());
  ...
  const resetDatabase = async () => {
    ...
    try {
      await axios.post('http://localhost:8000/search/pipeline/reset', {}, getAuthHeaders());
      await loadStats();
                  ...
                  <div style={{display: 'flex', alignItems: 'center', gap: '20px'}}>
                    ...
                    <div style={{flex: 1}}>
                      ...
                      <div style={{marginTop: '10px', fontSize: '14px'}}>
                        <strong>Redundancy Rate:</strong> {advancedResults.health_score?.redundancy_rate || 0}%
                      </div>
                    </div>
                  </div>
                  <div style={{
              ...
              <div className="card" style={{marginTop: '20px'}}>
                ...
                <div style={{padding: '20px'}}>
                  <div style={{marginBottom: '15px'}}>
                    <strong>Total Items Analyzed:</strong> {advancedResults.total_items || 0}
                    {' '} | {' '}
                    <strong>Total Relationships:</strong> {advancedResults.total_relationships || 0}
                  </div>

                  {/* Recommendations */}
                  {advancedResults.insights?.recommendations && (
                    <div style={{marginTop: '20px'}}>
                      <h3 style={{fontSize: '18px', marginBottom: '10px'}}>
                        <i className="fas fa-lightbulb"></i> Recommendations
                      </h3>
                      <ul style={{listStyle: 'none', padding: 0}}>
                        {advancedResults.insights.recommendations.map((rec, i) => (
                          <li key={i} style={{
                            padding: '10px',
                            background: '#f9fafb',
                            borderRadius: '6px',
                  ...
                  {/* Key Findings */}
                  {advancedResults.insights?.key_findings && advancedResults.insights.key_findings.length > 0 && (
                    <div style={{marginTop: '20px'}}>
                      <h3 style={{fontSize: '18px', marginBottom: '10px'}}>
                        <i className="fas fa-star"></i> Key Findings
                      </h3>
                      {advancedResults.insights.key_findings.map((finding, i) => (
                        <div key={i} style={{
                          padding: '15px',
                          background: '#f9fafb',
                          borderRadius: '8px',
                          ...
                          <h4 style={{fontSize: '16px', marginBottom: '10px', color: '#1f2937'}}>
                            {finding.title}
                          </h4>
                            ...
                            <ul style={{listStyle: 'none', padding: 0, marginTop: '10px'}}>
                              {finding.items.map((item, j) => (
                                <li key={j} style={{
                                  ...
                                  {item.pair && <span><strong>{item.pair}</strong> - {item.score}</span>}
                                  {item.name && <span><strong>{item.name}</strong> - {item.connections} connections</span>}
                                </li>
                              ))}
                            </ul>
                          )}
